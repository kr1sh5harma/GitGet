      ...
      <main className="max-w-5xl mx-auto px-5 pt-24 pb-20">
        ...
        <div className="flex flex-col md:flex-row items-start gap-8 mb-12">
          <Image
            ...
            unoptimized
            priority
            className="rounded-2xl border border-[#222] shrink-0"
          />
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-3 mb-1">
              <h1 className="text-2xl font-medium tracking-tight">{user.name || user.login}</h1>
              <span
            ...
            {user.bio && <p className="text-[14px] text-zinc-400 mb-4 max-w-lg leading-relaxed">{user.bio}</p>}
          ...
          <div className="flex flex-col sm:flex-row gap-2 shrink-0">
            <a
              ...
              className="text-xs font-mono text-zinc-500 border border-[#2a2a2a] px-4 py-2 rounded-lg hover:text-white hover:border-[#444] transition-all text-center"
            >
              View on GitHub ↗
            </a>
            <CopyLinkButton username={params.username} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-10">
          {[
            ...
            <div key={s.label} className="bg-[#111] border border-[#1e1e1e] rounded-xl p-4 text-center">
              <div className="text-xl font-light text-white mb-1">{s.value}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
          ...
          {/* Archetype Card */}
          <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-5 col-span-1">
            <div className="text-[11px] font-mono text-zinc-600 uppercase tracking-wider mb-4">Developer Archetype</div>
            <div className="text-lg font-medium mb-1" style={{ color: archetype.color }}>{archetype.label}</div>
            <div className="text-xs text-zinc-500 mb-4">{archetype.desc}</div>
            <div className="flex flex-wrap gap-1.5">
              {langs.slice(0, 4).map(l => (
                <span key={l.lang} className="flex items-center gap-1.5 text-[11px] font-mono text-zinc-500 bg-[#1a1a1a] border border-[#222] rounded-full px-2.5 py-1">
          ...
          <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-5">
            <div className="text-[11px] font-mono text-zinc-600 uppercase tracking-wider mb-4">Top Languages</div>
              ...
              {langs.map(l => (
                <div key={l.lang}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[11px] font-mono text-zinc-600">{l.count} repos</span>
                  </div>
          ...
          {/* Open Source Impact */}
          <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-5">
            <div className="text-[11px] font-mono text-zinc-600 uppercase tracking-wider mb-4">Open Source Impact</div>
            <div className="space-y-3 mb-4">
              <div className="flex justify-between">
                <span className="text-xs text-zinc-500">Stars earned</span>
                <span className="text-xs font-mono text-amber-400">{fmtNum(impact.totalStars)} ★</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-zinc-500">Times forked</span>
                <span className="text-xs font-mono text-blue-400">{fmtNum(impact.totalForks)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-zinc-500">Source repos</span>
                <span className="text-xs font-mono text-zinc-400">{sourceRepos.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-zinc-500">Years on GitHub</span>
                <span className="text-xs font-mono text-zinc-400">{yearsOnGH}y</span>
              </div>
            </div>
            {impact.topRepos[0] && (
              <div className="pt-3 border-t border-[#1a1a1a]">
                <div className="text-[10px] font-mono text-zinc-700 uppercase tracking-wider mb-2">Top repo</div>
                <Link
                  href={`/${params.username}/${impact.topRepos[0].name}`}
                  className="text-xs font-mono text-zinc-300 hover:text-white transition-colors flex items-center gap-1.5"
                >
